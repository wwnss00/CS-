...
그래프에서 모든 정점을 방문하는 대표적인 탐색 방법으로는 깊이우선탐색(DFS)과 너비우선탐색(BFS)이 있습니다.

## 1. 깊이우선탐색(DFS, Depth First Search)
그래프에서 더 이상 방문 가능한 정점이 없을 때까지 최대한 깊이 탐색하는 방식입니다.

### **특징**
- 스택(Stack) 또는 재귀(Recursion)를 활용하여 구현 가능
- 백트래킹(Backtracking) 기법과 함께 활용될 수 있음
- 한 경로를 끝까지 탐색한 후 다른 경로를 탐색

### **동작 과정**
1. 시작 정점을 방문하고 스택에 삽입
2. 스택의 최상단 정점을 가져와 방문
3. 방문한 정점과 연결된 방문하지 않은 정점을 스택에 삽입
4. 더 이상 방문할 정점이 없을 때까지 반복

---

## 2. 너비우선탐색(BFS, Breadth First Search)
최대한 넓게 탐색하는 방식을 반복하여 모든 정점을 방문하는 방법입니다.

### **특징**
- 큐(Queue)를 활용하여 구현 가능
- 최단 경로 문제에서 유용하게 사용됨 (특히, 가중치가 동일할 때)
- 계층적으로 탐색하는 방식

### **동작 과정**
1. 시작 정점을 방문하고 큐에 삽입
2. 큐에서 정점을 꺼내 방문
3. 방문한 정점과 인접한 방문하지 않은 정점을 큐에 삽입
4. 더 이상 방문할 정점이 없을 때까지 반복

---

## 3. 최단 경로 알고리즘
한 정점에서 목적지 정점까지 이르는 가중치의 합이 최소가 되는 경로를 찾는 알고리즘입니다.

### **다익스트라 알고리즘(Dijkstra Algorithm)**
특정 정점에서 다른 모든 정점까지의 최단 경로를 구하는 알고리즘으로, 간선의 가중치가 음이 아닌 경우에만 사용 가능합니다.

### **특징**
- **우선순위 큐(Priority Queue)** 또는 **배열(Array)** 을 활용하여 구현 가능
- 최단 거리 테이블을 이용하여 각 정점까지의 최소 비용을 갱신
- 다이나믹 프로그래밍(Dynamic Programming) 기반 접근 방식

### **동작 과정**
1. 최단 거리 테이블을 생성하고, 시작 정점을 제외한 모든 정점을 무한대로 초기화
2. 시작 정점을 방문하고 해당 정점의 거리를 0으로 설정
3. 방문한 정점과 인접한 정점들을 탐색
4. 기존 최단 거리와 새롭게 계산된 가중치를 비교하여 더 작은 값으로 갱신
5. 방문하지 않은 정점 중 최단 거리가 가장 작은 정점을 선택하여 방문
6. 더 이상 방문할 정점이 없을 때까지 3~5의 과정을 반복 후 종료

이와 같은 방식으로 그래프 탐색과 최단 경로 알고리즘을 활용하여 다양한 문제를 해결할 수 있습니다.
